...
# Servicio noVNC (websockify :8081 → localhost:5901)
cat > /etc/systemd/system/novnc.service << 'UNIT'
[Unit]
Description=noVNC proxy (websockify 8081 → VNC 5901)
...
echo "[gui] Entorno gráfico listo. Accede en http://localhost:8081/vnc.html (password: vagrant)"
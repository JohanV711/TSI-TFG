#!/usr/bin/env bash
# Configuración de máquina de rol de atacante externo Kali.
# Red 100.70.9.0/24, ip: 100.70.9.10. Sin salida a internet
set -euo pipefail
export DEBIAN_FRONTEND=noninteractive

# Forzar DNS funcional para evitar cuelgues
systemctl stop systemd-resolved 2>/dev/null || true
systemctl disable systemd-resolved 2>/dev/null || true
rm -f /etc/resolv.conf
echo "nameserver 1.1.1.1" > /etc/resolv.conf
echo "nameserver 1.0.0.1" >> /etc/resolv.conf

apt-get update -qq
apt-get install -y -qq \
  netdiscover \
  hydra \
  tcpdump \
  wireshark-common \
  dsniff \
  nmap \
  ettercap-text-only \
  telnet \
  ftp

# Detección dinámica de interfaces (gestión NAT vs. laboratorio)
sleep 5
MGMT_IFACE=$(ip route | awk '/^default/ {print $5; exit}')
LAB_IFACE=$(ip -br link show | awk -v mgmt="$MGMT_IFACE" '$1 != "lo" && $1 != mgmt {print $1; exit}')

if [ -z "$MGMT_IFACE" ] || [ -z "$LAB_IFACE" ]; then
  echo "ERROR: interfaces no detectadas. Interfaces actuales:"
  ip -br link show
  echo "Esperando 20 segundos adicionales..."
  sleep 20
  MGMT_IFACE=$(ip route | awk '/^default/ {print $5; exit}')
  LAB_IFACE=$(ip -br link show | awk -v mgmt="$MGMT_IFACE" '$1 != "lo" && $1 != mgmt {print $1; exit}')
fi

echo "MGMT=$MGMT_IFACE LAB=$LAB_IFACE"

# Configurar interfaz de gestión (NAT de Vagrant)
MGMT_CONN=$(nmcli -t -f NAME,DEVICE connection show --active | awk -F: -v ifc="$MGMT_IFACE" '$2==ifc {print $1; exit}')
nmcli connection modify "$MGMT_CONN" ipv4.route-metric 50 || true

# Configurar interfaz del laboratorio
nmcli connection delete "lab-externa" 2>/dev/null || true
nmcli connection add \
  type ethernet \
  ifname "$LAB_IFACE" \
  con-name "lab-externa" \
  ip4 100.70.9.10/24 \
  -- ipv4.method manual \
     ipv4.route-metric 101 \
     ipv4.never-default yes \
     ipv6.method disabled

nmcli connection modify "lab-externa" \
  +ipv4.routes "192.168.57.0/24 100.70.9.1" \
  +ipv4.routes "192.168.58.0/24 100.70.9.1"

nmcli connection up "$MGMT_CONN" || true
sleep 2
nmcli connection up "lab-externa" || true

sleep 3
ip route add 192.168.57.0/24 via 100.70.9.1 dev "$LAB_IFACE" 2>/dev/null || true
ip route add 192.168.58.0/24 via 100.70.9.1 dev "$LAB_IFACE" 2>/dev/null || true

echo "=== Configuración de red aplicada ==="
ip addr show "$LAB_IFACE"
echo "=== Rutas configuradas ==="
ip route show

# CONFIGURACIÓN DE SEGURIDAD (MALAS PRÁCTICAS)
sysctl -w net.ipv4.ip_forward=0
echo "net.ipv4.ip_forward=0" > /etc/sysctl.d/99-kali-lab.conf

mkdir -p /home/vagrant/lab/{recon,captures,exploits}
chown -R vagrant:vagrant /home/vagrant/lab

cat >> /home/vagrant/.bashrc << 'EOF'
# Insecure-network-lab aliases
alias nmap-quick='nmap -sV -T4 --open'
alias nmap-full='nmap -sV -sC -O -p- -T4'
alias iface='ip -br addr show'
alias targets='echo "DMZ: 192.168.57.10"; echo "Internal: 192.168.58.10"'
EOF

mkdir -p /home/vagrant/.ssh
chmod 700 /home/vagrant/.ssh
cat > /home/vagrant/.ssh/config << 'EOF'
Host 192.168.57.*
    StrictHostKeyChecking no
    UserKnownHostsFile /dev/null
Host 192.168.58.*
    StrictHostKeyChecking no
    UserKnownHostsFile /dev/null
EOF
chmod 600 /home/vagrant/.ssh/config
chown -R vagrant:vagrant /home/vagrant/.ssh

echo "[external-kali] completado"
